...
export function DependencyIndicator({ task, projectId }: DependencyIndicatorProps) {
  const { data: dependencies } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient.get(`/projects/${projectId}/tasks/${task.id}/dependencies`);
  ...
  return (
    <div className="mt-2 space-y-1">
      ...
      <div className="space-y-1">
        ...
        {deps.length > 3 && (
          <div className="font-mono text-[10px] text-text-tertiary">+{deps.length - 3} more</div>
        )}
      </div>
    </div>
  );
