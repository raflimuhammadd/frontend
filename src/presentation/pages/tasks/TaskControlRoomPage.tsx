...
export function TaskControlRoomPage() {
  ...
  const metrics = [
    ...
    },
    {
      ...
      subtext: `${Math.round(
        (tasksList.filter((t) => t.status === "IN_PROGRESS").length / (tasksList.length || 1)) *
          100,
      )}% workload`,
    },
    {
        ...
        {metrics.map((metric, index) => (
          <div key={index} className="bg-background-card px-4 py-5">
            <div className="flex justify-between items-start">
              ...
              {metric.icon && (
                <span
                  className="inline-block w-[16px] h-[16px] text-text-secondary"
          ...
          <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
            <div className="flex flex-wrap items-center gap-2">
              <div className="relative">
                <span
                  className="absolute left-3 top-2.5 text-text-secondary text-[15px]"
              ...
              <button className="h-9 px-3 text-[10px] uppercase tracking-wide-15 text-text-secondary flex items-center gap-2 hover:text-text-primary">
                <span
                  className="inline-block w-[14px] h-[14px]"
                  data-icon="lucide:arrow-up-down"
                  data-inline="false"
                />
                Recent
              </button>
            ...
            <div className="divide-y divide-border-light">
              {paginatedTasks.map((task) => (
                ...
                >
                  ...
                  <div className="font-mono text-[10px] text-text-secondary">— no blockers</div>

                  <div
                    className={`text-[10px] uppercase font-semibold ${getPriorityColor(task.priority)}`}
                  >
                    ● {task.priority}
                  </div>
